    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
        <div v-for="type of allTypes.data">
            <div class="animate-fade-in-down bg-white rounded-lg shadow flex flex-col items-center justify-center" style="animation-delay: 0.2s">
                <router-link :to="{name: 'app.car.view', params: {id: type.id}}" class="py-4 flex items-center w-full h-full rounded-lg justify-center transition-colors hover:text-white hover:bg-gray-500">
                <span class="text-3xl">
                    {{type.name}}
              </span>
                </router-link>
